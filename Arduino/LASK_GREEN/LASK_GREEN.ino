...
//int route[500] = {1, 4 , 2, ROTATE_180, ROTATE_180, ROTATE_180};
int route[600] = {1, 1, 4, 1, 4, 1, 8, 7, 1, 4, 1, 1, 4, 1, 9, 7, 2, 1, 2, 8, 7, 1, 4, 1, 1, 1, 4, 1, 1, 1, 1, 4, 1, 2, 9, 7, 2, 1, 1, 2, 8, 7, 4, 1, 4, 9, 7, 1, 4, 4, 1, 8, 7, 2, 1, 1, 1, 2, 1, 1, 2, 1, 2, 9, 7, 2, 1, 1, 2, 1, 1, 2, 1, 2, 8, 7, 2, 2, 9, 7, 2, 1, 2, 1, 1, 2, 2, 8, 7, 2, 1, 1, 1, 2, 1, 1, 2, 1, 2, 9, 7, 1, 1, 8, 7, 2, 4, 1, 9, 7, 1, 4, 1, 4, 1, 1, 1, 1, 4, 1, 1, 4, 1, 8, 7, 2, 1, 2, 1, 1, 2, 4, 1, 9, 7, 1, 4, 1, 1, 2, 8, 7, 1, 4, 1, 2, 9, 7, 1, 4, 1, 4, 1, 8, 7, 2, 4, 1, 9, 7, 2, 1, 2, 1, 1, 2, 2, 8, 7, 2, 2, 9, 7, 1, 4, 1, 1, 4, 1, 1, 1, 1, 1, 1, 1, 1, 4, 1, 2, STOP, STOP};
                  //2 ,1 ,4 ,1 ,1 ,1 ,1 ,4 ,1 ,1 ,4, 1 ,8 ,7 ,1 ,4 ,1 ,4 ,1 ,9 ,7 ,2 ,1 ,1 ,1 ,1 ,4 ,4 ,1 ,1 ,1 ,1 ,1 ,1 ,1 ,4, 4 ,1 ,1 ,1 ,1 ,1 ,1 ,1 ,4 ,4};
//{GO_F, GO_R, PAUSE, GO_L, GO_L, GO_L, GO_F, ROTATE_180, STOP}; //경로 노가다로 넣는 부분
int routeSize = 0;
...
void t_right(){ // 우회전
 ...
 // Serial.println(R_en_cnt);
  ...
  R_cur_cnt = knobRight.read();
  //Serial.println(R_en_cnt);
  while(abs(knobRight.read() - R_cur_cnt) <= 290){ // 우회전 
...
void t_left(){ //좌회전
 ...
 // Serial.println(L_en_cnt);
  ...
  L_cur_cnt = knobLeft.read();
  //Serial.println(L_en_cnt);
  while(abs(knobLeft.read() - L_cur_cnt) < 300){ 
    digitalWrite(A1_dir, HIGH);
    digitalWrite(A2_dir, LOW); 
    digitalWrite(B1_dir, LOW);
    digitalWrite(B2_dir, HIGH);
